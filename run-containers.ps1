...
try {
    ...
    if ($response.status -eq "healthy") {
        ...
        Write-Host "Backend health check returned: $($response.status)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "Backend is not responding yet (may need more time)" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "Frontend is not responding yet (may need more time)" -ForegroundColor Yellow
}
